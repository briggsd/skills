...
- [planf3](planf3/) — Author an engineering implementation plan as a single self-contained HTML page: phased task checklists with per-phase testing loops, a relevant-files map, surfaced questionables, and embedded section images sharing one visual identity. Append-only metadata and an amendments log let the same plan be created, updated, and built across sessions. Ships create/update/build workflows and optional GPT-image generation scripts (needs `OPENAI_API_KEY`).
